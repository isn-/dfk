...
{
  for (size_t i = 0; i < DFK_SIZE(f->values); ++i) {
...
static void myint_expect_equal(dfk_list_t* l, int* expected, size_t expectedlen)
{
  EXPECT(dfk_list_size(l) == expectedlen);
  size_t i = 0;
  dfk_list_it it, end;
  dfk_list_begin(l, &it);
  dfk_list_end(l, &end);
  while (!dfk_list_it_equal(&it, &end)) {
    EXPECT(((myint_t*) it.value)->value == expected[i++]);
    dfk_list_it_next(&it);
  }
  EXPECT(i == expectedlen);
}
...
TEST_F(myint_fixture, list, insert_begin)
{
  ...
  int expected[] = {6, 10, 11, 12};
  myint_expect_equal(l, expected, DFK_SIZE(expected));
}
...
TEST_F(myint_fixture, list, swap_full_full)
{
  ...
  dfk_list_swap(l, &dst);
  myint_expect_equal(l, dstintvalues, DFK_SIZE(dstintvalues));
  myint_expect_equal(&dst, fixture->int_values, DFK_SIZE(fixture->int_values));
}
